...
export default {
  ...
  async fetch() {
    await this.$store.dispatch("top/fetchByStoreBrand", {
  ...
  computed: {
    ...
    breadcrumbs() {
      ...
      if (this.storeBrandIndex) {
        items.push({
          ...
          url: `/${this.storeBrand.label.toLowerCase()}`,
        });
      }
    ...
    pageTitle() {
      if (this.storeBrandIndex) return `NEW ITEM (${this.storeBrand.label})`;
